...
durdraw_plugin = {
    ...
    "type": ["effect"],
    "desc": "Fades out a range of frames by darkening colors toward black, without changing characters."
}
...
def transform_movie(mov, appState=None, opts=opts):
    ...
    for i, frame_idx in enumerate(range(start - 1, end)):
        # brightness goes from 1.0 (first frame) down to near-zero (last frame).
        # We use a small floor so the last frame isn't pure black —
        # just very dark, preserving a hint of hue.
        brightness = (num_fade_frames - i) / num_fade_frames
