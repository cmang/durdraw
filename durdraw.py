...
class drawing():
   ...
   \_____________________________________________\|  v 0.1
...
'''
    ...
    def testWindowSize(self):
        ...
        if self.realmaxY < self.maxY or self.realmaxX < self.maxX:
    ...
    def smallWindowMode(self):
        ...
        self.stdscr.refresh()
        self.realmaxY,self.realmaxX = self.stdscr.getmaxyx()
        while (self.realmaxY < self.maxY or self.realmaxX < self.maxX):
    ...
    def refresh(self):          # Why doesn't this always work right?
        """Refresh the screen"""
        linenum = 0
        for line in self.content:
            #for colnum in range(len(line)):    # maxX = len(line), or should be
            for colnum in range(self.maxX):
    ...
    def altLeft(self):          # This causes ghost characters when refresh()
        """Erase column at position of cursor"""
        for x in range(len(self.content)):         # Pop current column from every
            self.content[x].pop(self.xy[1] - 1)     # line & add a blank
    ...
    def altRight(self):
        ...
        for x in range(len(self.content)):
            self.content[x].insert(self.xy[1] - 1, ' ')
            self.content[x].pop()
        self.refresh()
    ...
    def altDown(self):          # This causes ghost characters when refresh()
        """Insert new line"""
